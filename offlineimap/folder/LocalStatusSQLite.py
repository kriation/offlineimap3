...
class LocalStatusSQLiteFolder(BaseFolder):
    ...
    def openfiles(self):
        ...
        assert (
            sqlite.threadsafety == self._threading_mode_const
    ...
    def __upgrade_db(self, from_ver):
        ...
        if from_ver <= 1:
            self.ui._msg(
                "Upgrading LocalStatus cache from version 1 to version 2 for %s:%s"
    ...
    def cachemessagelist(self):
        ...
        for row in cursor:
            ...
            try:
                labels = set(
                    [lb.strip() for lb in row[3].split(",") if len(lb.strip()) > 0]
                )
    ...
    def savemessage(self, uid, msg, flags, rtime, mtime=0, labels=None):
        ...
        try:
            ...
            raise UserWarning(
                "%s while inserting UID %s" % (str(e), str(uid)), exc_info()[2]
            )
        return uid
    ...
    def savemessagelabels(self, uid, labels, mtime=None):
        ...
        if mtime:
            self.__sql_write(
                "UPDATE status SET labels=?, mtime=? WHERE id=?", (labels, mtime, uid)
            )
        else:
