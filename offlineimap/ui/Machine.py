...
class MachineUI(UIBase):
    ...
    def syncfolders(self, srcrepos, destrepos):
        self._printData(
            self.logger.info,
            "syncfolders",
            "%s\n%s" % (self.getnicename(srcrepos), self.getnicename(destrepos)),
        )
    ...
    def loadmessagelist(self, repos, folder):
        self._printData(
            self.logger.info,
            "loadmessagelist",
            "%s\n%s" % (self.getnicename(repos), folder.getvisiblename()),
        )
    ...
    def messagelistloaded(self, repos, folder, count):
        self._printData(
            self.logger.info,
            "messagelistloaded",
            "%s\n%s\n%d" % (self.getnicename(repos), folder.getname(), count),
        )
    ...
    def syncingmessages(self, sr, sf, dr, df):
        self._printData(
            ...
            "syncingmessages",
            "%s\n%s\n%s\n%s\n"
            % (self.getnicename(sr), sf.getname(), self.getnicename(dr), df.getname()),
        )

    def ignorecopyingmessage(self, uid, srcfolder, destfolder):
        self._printData(
            self.logger.info,
            "ignorecopyingmessage",
            "%d\n%s\n%s\n%s[%s]"
            % (
                uid,
                self.getnicename(srcfolder),
                srcfolder.getname(),
                self.getnicename(destfolder),
                destfolder,
            ),
        )
    ...
    def deletingmessages(self, uidlist, destlist):
        ...
        self._printData(
            self.logger.info, "deletingmessages", "%s\n%s" % (self.uidlist(uidlist), ds)
        )
    ...
    def addingflags(self, uidlist, flags, dest):
        self._printData(
            self.logger.info,
            "addingflags",
            "%s\n%s\n%s" % (self.uidlist(uidlist), "\f".join(flags), dest),
        )
    ...
    def threadException(self, thread):
        self._printData(
            self.logger.warning,
            "threadException",
            "%s\n%s" % (thread.getName(), self.getThreadExceptionString(thread)),
        )
    ...
    def mainException(self):
        self._printData(
            self.logger.warning, "mainException", self.getMainExceptionString()
        )
