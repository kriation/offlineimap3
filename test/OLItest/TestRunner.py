...
class OLITestLib:
    ...
    def __init__(self, cred_file=None, cmd="offlineimap"):
        ...
        if not os.path.isfile(cred_file):
            raise UserWarning(
                "Please copy 'credentials.conf.sample' to '%s' "
    ...
    def create_test_dir(cls, suffix=""):
        ...
        cls.testdir = os.path.abspath(
            tempfile.mkdtemp(
                prefix="tmp_%s_" % suffix, dir=os.path.dirname(cls.cred_file)
            )
        )
    ...
    def run_OLI(cls):
        ...
        try:
            output = subprocess.check_output(
                [cls.cmd, "-c%s" % os.path.join(cls.testdir, "offlineimap.conf")],
                shell=False,
            )
    ...
    def delete_remote_testfolders(cls, reponame=None):
        ...
        for sec in sections:
            ...
            dirs = [
                d
                for d in dirs
                if d.startswith('"INBOX.OLItest') or d.startswith('"INBOX/OLItest')
            ]
            for folder in dirs:
                res_t, data = imapobj.delete(folder)
                assert (
                    res_t == "OK"
                ), "Folder deletion of {0} failed with error" ":\n{1} {2}".format(
                    folder.decode("utf-8"), res_t, data
                )
            imapobj.logout()
